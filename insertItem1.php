<?php
	...
	if(isset($_POST['itemDetailsItemNumber1'])){
		...
		if(!empty($ItemNumber1) && !empty($ItemName2) && isset($Discount3) && isset($Quantity4)&& isset($ItemNumber5)&& isset($ItemNumber6)&& isset($Quantity7)&& isset($Quantity8)&& isset($ItemNumber9)&& isset($Quantity10)){
			...
			if(filter_var($ItemName2, FILTER_VALIDATE_INT) === 0 || filter_var($ItemName2, FILTER_VALIDATE_INT)||filter_var($Quantity7, FILTER_VALIDATE_INT) === 0 || filter_var($Quantity7, FILTER_VALIDATE_INT)||filter_var($Quantity8, FILTER_VALIDATE_INT) === 0 || filter_var($Quantity8, FILTER_VALIDATE_INT)){
				// Valid quantity
			} else {
...
/// ajouter une autre condition username
...
if(sqlsrv_has_rows($result) != 1){
	...
	// Encrypt the password
			// $Quantity10 = mdn5($Quantity10);

            $Quantity10a = password_hash($Quantity10, PASSWORD_BCRYPT);

 
                         $insertItemSql=  "INSERT INTO dbo.[agent]([code_centre],[code_agent], [nom_agent], [prenom_agent],[poste],[etat],[num_cle], [num_puce], [username],[password])
                                           VALUES('$ItemNumber1','$ItemName2', '$Discount3','$Quantity4','$ItemNumber5','$ItemNumber6','$Quantity7', '$Quantity8','$ItemNumber9','$Quantity10a')";
                        ...
                        $insertItemSql1=  "INSERT INTO dbo.[user]([username],[password],[nom_agent],[prenom_agent])
                                           VALUES('$ItemNumber9','$Quantity10a','$Discount3','$Quantity4')";
